...
####Packages needed
package_names= c("MASS","alr3","car","relimp","fortunes","mfp","ggplot2","GGally","gridExtra","leaps","elasticnet","caret")
...
#OR
WeightsPlot <- ggplot(Brains, aes(x=BodyWt, y=BrainWt)) + geom_point(alpha= 0.5)

print(WeightsPlot)

# I like the ggplot output :-)

# Let's fit a regression line
ggplot(Brains, aes(x=BodyWt, y=BrainWt)) + geom_point(alpha= 0.5) + geom_smooth(method=lm)
# We can see that this way it's very difficult to make predictions.
# The shape of the plot implies a relationship in the log scale though
## ----brains-log-----------------
plot(log(Brains$BodyWt),log(Brains$BrainWt))
#OR
LogWeightsPlot <- qplot(log(BodyWt), log(BrainWt), data = Brains, alpha = I(0.5))
#OR
LogWeightsPlot <- ggplot(Brains, aes(x=log(BodyWt), y=log(BrainWt))) + geom_point(alpha= 0.5) + geom_smooth(method=lm)
...
## The same can be done with the geom_abline if we get the Coefficients

## brains Log-Linear Model
# Fit the simple linear model to the log brain weight data
...
summary(Brains)
range(Brains)
BodyWtGrid <- seq(range(Brains)[1], range(Brains)[2], length.out = 100)
