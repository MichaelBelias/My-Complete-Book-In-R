...
## ---- Question A3 ----
coins.jag <- jags.model("coins-model.txt", quiet=TRUE)
...
drug.coda <- coda.samples(drug.jag, c("theta","r","P.crit"),
                          n.iter=30000)
summary(drug.coda)
...
drug.B2.jag <- jags.model("solutions/practical1/drug-MC-model-B2.txt",
                          quiet=TRUE)
drug.B2.coda <- coda.samples(drug.B2.jag, c("theta","r","P.crit"),
                             n.iter=30000)
drug.B2.coda.r <- drug.B2.coda[[1]][,"r"]
barplot(prop.table(table(drug.B2.coda.r)))
...
t.D1.coda <- coda.samples(t.D1.jag, "y", n.iter=10000)
summary(t.D1.coda)
densplot(t.D1.coda)
...
cubed.D2.coda <- coda.samples(cubed.D2.jag, c("y","ycubed"), n.iter=10000)
...
densplot(drug.coda[, "theta"]) # thete is a continuous parameter
autocorr.plot(drug.coda, auto.layout=FALSE)
