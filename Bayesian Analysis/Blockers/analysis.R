...
block <- stan("blocker.stan", data = "blocker.data.R", iter=1000,chains=4)

sm <- stan_model(model_code = "blocker.stan")

summary(block)



post <- read.csv(file = "blocker.csv", header = TRUE, comment.char = '#'); 
